...
def main(_):
    ...
    pathname = "model/ATT_GRU_model-10000"
    ...
    test_settings.batch_size = 262 * 9

    batch_size_test = test_settings.batch_size
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            def eval_pn(test_y, test_word, test_pos1, test_pos2, test_settings):
                allprob = []
                acc = []
                for i in range(int(len(test_word) / float(test_settings.batch_size))):
                    prob, accuracy = test_step(
                        test_word[i * test_settings.batch_size:(i + 1) * test_settings.batch_size],
