...
class GRU:
    def __init__(self, is_training, word_embeddings, settings):
        ...
        inputs_forward = tf.concat(axis=2, values=[
            tf.nn.embedding_lookup(word_embedding, self.input_word),  # ? * 70 * 50
            tf.nn.embedding_lookup(pos1_embedding, self.input_pos1),  # ? * 70 * 5
        ...
        for i in range(big_num):
            ...
            with tf.name_scope("loss"):
                self.loss.append(tf.reduce_mean(
                    tf.nn.softmax_cross_entropy_with_logits(logits=sen_out[i], labels=self.input_y[i])))
                if i == 0:
